...
public class PageTiyu extends BaseIndexPage {
    ...
    public PageTiyu(Context context) {
    ...
    * */
    public void dataFromNet() {
        ...
        HttpUtil.sendHttpRequest(Constant.urlTiyu, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Activity activity = (Activity) PageTiyu.this.context;
                activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtil.show(PageTiyu.this.context, "可能没网了");
                    }
                });
            }
