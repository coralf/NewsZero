...
public class PageYule extends BaseIndexPage {
    ...
    public PageYule(Context context) {
    ...
    * */
    public void dataFromNet() {
        ...
        HttpUtil.sendHttpRequest(Constant.urlYule, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Activity activity = (Activity) PageYule.this.context;
                activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtil.show(PageYule.this.context, "可能没网了");
                    }
                });
            }
